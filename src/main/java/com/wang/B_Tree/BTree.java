...
public class BTree {
    ...
    public boolean removeKey(Integer key){
        ...
        if(node.sizeOfChildren() != 0){            //非叶子结点
            ...
            BTNode greatest = this.getGreatestNode(left);  //从上到下直到叶节点找到最大的叶结点
            ...
            //3)既是叶子结点又是根节点且node.sizeOfKeys() > Constants.MIN_KEY_SIZE
            //4)叶子结点但非根结点且node.sizeOfKeys() > Constants.MIN_KEY_SIZE
            if(node.getParent() != null && node.sizeOfKeys() < Constants.MIN_KEY_SIZE) {
                this.combined(node);
            }else if(node.getParent() == null && node.sizeOfKeys() == 0) {
                //删除的是最后一个元素
                root = null;
            }
    ...
    private void combined(BTNode node) {
        //先获取此结点的父节点
        BTNode parentNode = node.getParent();
        //获取此结点是其父节点中的索引，即第几个孩子
        int index = parentNode.getChildNodes().indexOf(node);
        int indexOfLeftNeighbor = index - 1;   //获取此节点的左兄弟节点索引
        int indexOfRightNeighbor = index + 1;  //获取此节点的右兄弟节点索引

        BTNode rightNeighbor = null;
        int rightNeighborSize = 0;
        if(indexOfRightNeighbor < parentNode.sizeOfChildren()){   //右邻居存在
            rightNeighbor = parentNode.getChild(indexOfRightNeighbor);  //获取右兄弟衡量
            rightNeighborSize = rightNeighbor.sizeOfKeys();        //获取右兄弟有多少个key
        }

        //右邻居存在且其关键字个数大于最小值
        if(rightNeighbor != null && rightNeighborSize > Constants.MIN_KEY_SIZE) {
            //相当于左旋
            Integer removeValue = rightNeighbor.getKeys().remove(0);     //删除右兄弟节点中的第一个Key
            int prev = getIndexOfPreviousValue(parentNode, removeValue); //返回parentNode结点中值不大于但最接近与removeValue的关键字的索引
            Integer parentValue = parentNode.getKeys().remove(prev);     //删除父节点中对应的节点
            node.addKey(parentValue);                                    //将父节点中删除的key加入当前节点
            parentNode.addKey(removeValue);                               //将右兄弟中的第一个key加入父节点实现最终的左旋
            ...
            if(leftNeighbor != null && leftNeighborSize > Constants.MIN_KEY_SIZE){   //左邻居存在且其关键字个数大于最小值
                ...
                if(parentNode.getParent() != null && parentNode.sizeOfKeys() < Constants.MIN_KEY_SIZE){//还没到达根节点
                    this.combined(parentNode);

                }else if(parentNode.sizeOfKeys() == 0){
                    //父节点中没有关键字了，则降低树的高度
                    node.setParent(null);                          //注意：树的高度降低一层，此结点就变为根节点，一定要设置其父节点为null
                    parentNode = null;
                    root = node;
                }
    ...
    private void splitNode(BTNode node) {
        ...
        if(node.getParent() != null){        //有父节点
            BTNode parent = node.getParent();//取得其父节点
